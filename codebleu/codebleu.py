...
def calc_codebleu(
    ...
    # preprocess inputs
    references = [
        [x.strip() for x in ref] if type(ref) == list else [ref.strip()]
    ...
    keywords = [
        x.strip()
        for x in open(keywords_dir / (lang + ".txt"), "r", encoding="utf-8").readlines()
    ]
    ...
    syntax_match_score = syntax_match.corpus_syntax_match(
        references, hypothesis, lang, lang_so_file
    )
